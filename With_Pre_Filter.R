...
hist(dat$dfstim, main="Distribution of Survival Time", xlab="Survival Time",col="green")

fit_1 = survfit((Surv(data_subset$dfstim,data_subset$dfsind))~data_subset$tumor.subtype)
plot(fit_1, col=c("red","blue", "green"), xlab = "Progression free survival time in
months",ylab = "survival probabilities", main = "Progression free survival by Type")
legend("topright", lty = rep("solid", 2), legend =
         c("bone","cartilage","STS"),cex=0.5,
       col = c("red","blue", "green"))

fit_2 = survfit((Surv(data_subset$dfstim,data_subset$dfsind))~data_subset$grade)
plot(fit_2, col=c("red","blue", "green"), xlab = "Progression free survival time in
months",ylab = "survival probabilities", main = "Progression free survival by grade")
legend("topright", lty = rep("solid", 2), legend =
         c("high","intermediate","low"),cex=0.5,
       col = c("red","blue", "green"))

fit_3 = survfit((Surv(data_subset$dfstim,data_subset$dfsind))~data_subset$sex)
plot(fit_3, col=c("blue", "green"), xlab = "Progression free survival time in
months",ylab = "survival probabilities", main = "Progression free survival by Sex")
legend("topright", lty = rep("solid", 2), legend =
         c("Female","Male"),cex=0.5,
       col = c("blue", "green"))

fit_4 = survfit((Surv(data_subset$dfstim,data_subset$dfsind))~1, data = data_subset)
plot(fit_4, xlab = "Progression free survival time in
months",ylab = "Progressioin free survival probabilities", main = "Overall Progression free survival")
legend("topright", lty = rep("solid", 2), legend =
         c("Female","Male"),cex=0.5,
       col = c("blue", "green"))

dist_sex = table(dat$sex)
barplot(dist_sex, names.arg = c("Female","Male"), col=c("blue","green"), main="Gender")

dist_grade = table(dat$grade)
barplot(dist_grade, names.arg = c("High","Intermediate","Low"), col=c("blue","green","yellow"), main="Grade")

dist_subtype = table(dat$tumor.subtype)
barplot(dist_subtype, names.arg = c("STS","Bone","Cartilage"), col=c("blue","green","yellow"), main="Tumour Subtype")

dist_dfsind = table(dat$dfsind)
barplot(dist_dfsind,col=c("blue","green"), main="Progression Index")
...
# To calculate the threshold_limit
...
for(a in 1:repeats){
  ...
  for(b in 1:length(cvIndex)){
    training_dat = data_subset[cvIndex[[b]],]
    test_dat = data_subset[-cvIndex[[b]],]
    
    start_cox = coxph(Surv(dfstim,dfsind) ~ 1, data = training_dat)
    full_cox = coxph(Surv(dfstim,dfsind) ~ ., data = training_dat)
    ...
    stp_cox_fit = coxph(full_form_1,data=training_dat)
    ...
    final_stp_train_con[counter]<-concordance(stp_cox_fit, newdata = training_dat)$concordance
    final_stp_test_con[counter] <- concordance(stp_cox_fit, newdata = test_dat)$concordance
    
    x.train = subset(training_dat, select = -c(dfstim, dfsind))
    y.train = subset(training_dat, select = c(dfstim, dfsind))
    ...
    gbm_pred_train = predict(gbm_model, newdata=training_dat, type="link")
    ...
    temp_rsf_model = rfsrc(Surv(dfstim, dfsind) ~ .,data = training_dat, importance = TRUE)
...
# Random Forest survival with RFE
...
while(length(model_under_consideration)>2){
  ...
  orig_training_dat = shuffled_orig_dat[cvIndex1[[b]],]
  orig_test_dat = shuffled_orig_dat[-cvIndex1[[b]],]
  orig.surv.train = Surv(orig_training_dat$dfstim,orig_training_dat$dfsind)
  orig.surv.test = Surv(orig_test_dat$dfstim,orig_test_dat$dfsind)
  model_rfsrc=rfsrc(Surv(dfstim,dfsind) ~ .,data = orig_training_dat, importance = TRUE)
...
##boxplots for all the models


boxplot(unlist(final_stp_train_con),lasso_train_con,
        ridge_train_con,gbm.train.con,unlist(rsf_con_train),rfe_train_con,
...
boxplot(unlist(final_stp_test_con),lasso_test_con,
        ridge_test_con,gbm.test.con,unlist(rsf_con_test),rfe_test_con,
        names=c("Stepwise","Lasso","Ridge", "GBM","RSF","RFE"),
...
ridge_mean_predict = as.array(apply(ridge_pred_matrix,2,mean))
ridge_kmo = split.kms(ridge_mean_predict, os=data_subset$dfstim, oevn=data_subset$dfsind)
ridge_tmp_var = ridge_kmo$iz
ridge_km.split = survfit(Surv(data_subset$dfstim, data_subset$dfsind)~ridge_tmp_var, data=data_subset)
...
cor(dat$Reg.grad.min,dat$sum.var_GLCM)
